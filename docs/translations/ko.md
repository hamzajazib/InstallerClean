...
|   Either a patch file wouldn't say which patch it is, or it wouldn't say which programs it's for, or Windows wouldn't say in full which programs have that patch. |   어떤 패치 파일이 어느 패치인지 밝히지 않았거나, 어느 프로그램용인지 밝히지 않았거나, Windows가 그 패치가 적용된 프로그램을 모두 밝히지는 않았습니다. |
